...
At this point I would highly recommend integration testing, more and more the test will be for the humans. 
Code coverage at this point is meaningless to me, we want to now make sure our features are covered by tests and they break when the feature breaks. 
You are going to break existing features. It’s inevitable with the tools we are using, our new goal is to know asap. 
A lot of AI tools will look at the compiler errors and fix that in a loop but it will not verify existing workflows are not broken long as the code compiles. When we have failing tests we can feed that back into the model as well. Don’t forget you can’t trust the models it will sometimes change the test to pass and the test would lose its value.
